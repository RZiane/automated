...
### XML processing ##############################################
def clearAttrib(input_file, output_file):
    # Liste des attributs à conserver
    allowed_attributes = {"join", "n", "head", "function", "lemma", "udpos", "prpos", "uppos"}

    # Charger le fichier XML
    tree = ET.parse(input_file)
    root = tree.getroot()

    # Parcourir tous les éléments 'w' et filtrer leurs attributs
    for elem in root.iter('w'):
        # Conserver seulement les attributs autorisés
        new_attrib = {key: value for key, value in elem.attrib.items() if key in allowed_attributes}
...
def valid_xml(inputfile):
    ...
    path_CorrTable = 'HT_CRISCO-main/workflow_HT/ressources/MICLE_CorrTable_coherence_26.10.23.csv'

    list_coherence = make_coherence_table(path_CorrTable)

    print(list_coherence)
                    ...
                    for para in section.findall('.//p'):
                        ...
                        for sentence in para.findall('.//s'):
                            ...
                            for word in sentence.findall('.//w'):
                                ...
                                try:
                                    dictcorr_head = {"corr_head": word.get('corr_head')}
                                except:
                                    dictcorr_head = {"corr_head": "_"}
                                if str(dictcorr_head["corr_head"]) == "None":
                                    corr_head = "_"
                                else:    
                                    corr_head = str(dictcorr_head["corr_head"]).rstrip('\n')

                                try:
                                    dictcorr_fct = {"corr_fct": word.get('corr_fct')}
                                except:
                                    dictcorr_fct = {"corr_fct": "_"}
                                if str(dictcorr_fct["corr_fct"]) == "None":
                                    corr_fct = "_"
                                else:    
                                    corr_fct = str(dictcorr_fct["corr_fct"]).rstrip('\n')
                                ...
                                if corr_fct != '_':
                                    s_misc+='|corr_fct='+corr_fct
                                
                                if corr_head != '_':
                                    s_misc+='|corr_head='+corr_head

                                form = get_word_form(word)
                                # print(form)
                                # gold_pos
                                # mot = word_nb+"\t"+form.replace("\t", "").replace("\n", "")+"\t"+lemma+"\t_\t_\t_\t"+head+"\t"+function+"\t_\t"+s_misc+"\n"
                                mot = word_nb+"\t"+form.replace("\t", "").replace("\n", "")+"\t"+lemma+"\t"+udpos+"\t_\t_\t"+head+"\t"+function+"\t_\t"+s_misc+"\n"
...
def conversion_conllu2xml(inputfile, outputfile):
    ...
    for word in words:
        ...
        # Filtrer les attributs pour exclure ceux avec une valeur d'underscore
        filtered_attribs = {k: v for k, v in attribs.items() if v != '_'}

        mot = ET.Element('w', attrib=filtered_attribs)
